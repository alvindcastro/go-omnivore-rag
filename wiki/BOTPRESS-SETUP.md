...
**Nice to know:**
- `botId` and `clientId` are the same value — both are the Botpress shareable bot ID.
- `showPoweredBy: false` hides the "Powered by Botpress" badge.
- `enableConversationDeletion: true` lets the user clear conversation history from the widget.
- The widget is loaded from Botpress CDN — no self-hosting required.
- The `inject.js` URL is Botpress v2. If Botpress releases v3, the URL will change. The widget version is tied to the bot's published configuration.

---

## Testing the flow without the demo page

You can test the Botpress bot directly at its shareable URL:

```
https://cdn.botpress.cloud/webchat/v2/shareable.html?botId=3b6cf557-bc0a-4197-b16a-29c79706809f
```

This is useful for testing the flow independently of the demo page.
